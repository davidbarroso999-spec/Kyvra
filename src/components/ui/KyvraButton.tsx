...
export function KyvraButton({
  ...
  const hasNeon = variant === "album" || variant === "primary";

  return (
    <motion.button
      whileTap={{ scale: 0.95 }}
      className={cn(
        "relative flex items-center justify-center overflow-visible transition-all duration-300 group",
        isHardware && [
          "border border-white/10",
          "shadow-[inset_0_2px_4px_rgba(0,0,0,0.8),_inset_0_-1px_1px_rgba(255,255,255,0.05),_0_6px_10px_rgba(0,0,0,0.5)]",
          "hover:border-white/20 hover:shadow-[inset_0_2px_4px_rgba(0,0,0,0.8),_inset_0_-1px_1px_rgba(255,255,255,0.1),_0_8px_16px_rgba(0,0,0,0.6)]"
        ],
        variant === "album" ? "bg-transparent text-white font-sans font-medium" :
        className && className.includes("bg-") ? "" : "bg-transparent text-text-high",
    ...
    >
      ...
      {isHardware && (
        <div 
          className="absolute inset-0 opacity-[0.25] mix-blend-overlay pointer-events-none rounded-[inherit]" 
